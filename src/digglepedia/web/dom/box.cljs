(ns digglepedia.web.dom.box
  (:require [clojure.string :as string]))

;;
;; NOTE: This was borrowed from Day8/re-com, with only minor modifications
;;
;; https://raw.githubusercontent.com/Day8/re-com/master/src/re_com/box.cljs
;;
;; TODO: replace with code optimized for my use-cases, if I end up liking the approach.
;;
...
(defn h-box
  ...
  [& {:keys [size flex-wrap width height min-width min-height max-width max-height justify align align-self margin padding gap children class style attr]
      :or   {size "none" justify :start align :stretch}
      :as   args}]
  (let [s        (merge
                   (flex-flow-style "row nowrap")
                   (flex-child-style size)
                   (when flex-wrap   (flex-flow-style "row wrap"))
                   (when width      {:width      width})
                   (when height     {:height     height})
                   (when min-width  {:min-width  min-width})
                   (when min-height {:min-height min-height})
                   ...
                   style)
        gap-form (when gap (digglepedia.web.dom.box/gap
                             :size gap
